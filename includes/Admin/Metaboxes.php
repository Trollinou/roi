<?php
/**
 * Custom Meta Boxes and Fields for the ROI CPTs.
 *
 * @package ROI
 */
...
/**
 * Class Metaboxes
 * Handles registration and saving of metaboxes for Lecon, Exercice, and Cours.
 */
class Metaboxes {
	...
	public function init(): void {
		...
		add_action( 'add_meta_boxes', [ $this, 'add_lecon_meta_boxes' ] );
		add_action( 'save_post_roi_lecon', [ $this, 'save_lecon_meta' ] );

		add_action( 'add_meta_boxes', [ $this, 'add_exercice_meta_boxes' ] );
		add_action( 'save_post_roi_exercice', [ $this, 'save_exercice_meta' ] );

		add_action( 'add_meta_boxes', [ $this, 'add_cours_meta_boxes' ] );
		add_action( 'save_post_roi_cours', [ $this, 'save_cours_meta' ] );
	 ...
	 * @return void
	 */
	public function add_lecon_meta_boxes(): void {
		add_meta_box(
			'roi_lecon_details_metabox',
			__( 'Détails de la leçon', 'roi' ),
			[ $this, 'render_lecon_details_metabox' ],
			'roi_lecon',
			'normal',
			'high'
		);
	 ...
	 * @return void
	 */
	public function render_lecon_details_metabox( \WP_Post $post ): void {
	 ...
	 * @return void
	 */
	public function save_lecon_meta( int $post_id ): void {
		...
		if ( empty( $_POST['roi_difficulty'] ) ) {
			...
			remove_action( 'save_post_roi_lecon', [ $this, 'save_lecon_meta' ] );
			wp_update_post( [
				'ID'          => $post_id,
				'post_status' => 'draft',
			] );
	 ...
	 * @return void
	 */
	public function add_exercice_meta_boxes(): void {
		add_meta_box(
			'roi_exercice_details_metabox',
			__( 'Détails de l\'exercice', 'roi' ),
			[ $this, 'render_exercice_details_metabox' ],
			'roi_exercice',
			'normal',
			'high'
		);
	 ...
	 * @return void
	 */
	public function render_exercice_details_metabox( \WP_Post $post ): void {
	 ...
	 * @return void
	 */
	public function save_exercice_meta( int $post_id ): void {
		...
		if ( empty( $_POST['roi_difficulty'] ) ) {
			...
			remove_action( 'save_post_roi_exercice', [ $this, 'save_exercice_meta' ] );
			wp_update_post( [
				'ID'          => $post_id,
				'post_status' => 'draft',
			] );
	 ...
	 * @return void
	 */
	public function add_cours_meta_boxes(): void {
		add_meta_box(
			'roi_cours_builder_metabox',
			__( 'Constructeur de Cours', 'roi' ),
			[ $this, 'render_cours_builder_metabox' ],
			'roi_cours',
			'normal',
			'high'
		);
	 ...
	 * @return void
	 */
	public function render_cours_builder_metabox( \WP_Post $post ): void {
	 ...
	 * @return void
	 */
	public function save_cours_meta( int $post_id ): void {
		...
		if ( empty( $_POST['roi_difficulty'] ) ) {
			...
			remove_action( 'save_post_roi_cours', [ $this, 'save_cours_meta' ] );
			wp_update_post( [
				'ID'          => $post_id,
				'post_status' => 'draft',
			] );
	...
	public function enqueue_admin_scripts( string $hook ): void {
		...
		if ( ( 'post.php' === $hook || 'post-new.php' === $hook ) && isset( $post->post_type ) && in_array( $post->post_type, [ 'roi_exercice' ], true ) ) {
