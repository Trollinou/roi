...
const PluginDocumentSettingPanel = EditorPluginDocumentSettingPanel || EditPostPluginDocumentSettingPanel;
...
const LeconSettingsPanel = () => {
	const postType = useSelect( ( select ) =>
		select( 'core/editor' ) ? select( 'core/editor' ).getCurrentPostType() : null
	);
	...
	const niveau = useSelect( ( select ) => {
		...
		if ( editedMeta && editedMeta._roi_lecon_niveau !== undefined && editedMeta._roi_lecon_niveau !== null && editedMeta._roi_lecon_niveau !== '' ) {
			return String( editedMeta._roi_lecon_niveau );
		}
		const currentMeta = editor.getCurrentPostAttribute( 'meta' );
		if ( currentMeta && currentMeta._roi_lecon_niveau !== undefined && currentMeta._roi_lecon_niveau !== null && currentMeta._roi_lecon_niveau !== '' ) {
	...
	document.addEventListener('DOMContentLoaded', () => {
		if (
			...
			if ( dispatch( 'core/editor' ) && dispatch( 'core/editor' ).unlockPostSaving ) {
				dispatch( 'core/editor' ).unlockPostSaving( 'roi_lecon_missing_fields' );
			}
			if ( dispatch( 'core/notices' ) && dispatch( 'core/notices' ).removeNotice ) {
				dispatch( 'core/notices' ).removeNotice( 'roi_lecon_missing_fields_notice' );
			}
