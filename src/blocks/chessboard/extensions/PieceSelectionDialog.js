...
export class PieceSelectionDialog extends Extension {
	...
	constructor( chessboard ) {
		...
		this.registerExtensionPoint(
			...
			this.extensionPointRedrawBoard.bind( this )
		);
		chessboard.showPieceSelectionDialog =
			this.showPieceSelectionDialog.bind( this );
		chessboard.isPieceSelectionDialogShown =
			this.isPieceSelectionDialogShown.bind( this );
		chessboard.closePieceSelectionDialog =
			this.closePieceSelectionDialog.bind( this );
		this.pieceSelectionDialogGroup = Svg.addElement(
			chessboard.view.interactiveTopLayer,
			'g',
			{ class: 'piece-selection-dialog-group' }
		);
		this.state = {
	 ...
	 * @return {boolean} True if the dialog is shown or requested, false otherwise.
	 */
	isPieceSelectionDialogShown() {
		return (
			this.state.displayState === DISPLAY_STATE.shown ||
	...
	drawPieceButton( piece, point ) {
		...
		this.chessboard.view.drawPiece(
			this.pieceSelectionDialogGroup,
			piece,
			point
		);
	}
	...
	redrawDialog() {
		while ( this.pieceSelectionDialogGroup.firstChild ) {
			this.pieceSelectionDialogGroup.removeChild(
				this.pieceSelectionDialogGroup.firstChild
			);
		}
		if ( this.state.displayState === DISPLAY_STATE.shown ) {
			const squareWidth = this.chessboard.view.squareWidth;
			const squareHeight = this.chessboard.view.squareHeight;
			const squarePoint = this.chessboard.view.squareToPoint(
				this.state.dialogParams.square
			);
	...
	pieceSelectionDialogOnCancel( event ) {
		if ( this.state.displayState === DISPLAY_STATE.shown ) {
			...
			if ( this.state.callback ) {
				this.state.callback( {
					type: PIECE_SELECTION_DIALOG_RESULT_TYPE.canceled,
				} );
			}
		}
	}
	...
	setDisplayState( displayState ) {
		...
		if ( displayState === DISPLAY_STATE.shown ) {
			...
			this.chessboard.view.svg.addEventListener(
				'contextmenu',
