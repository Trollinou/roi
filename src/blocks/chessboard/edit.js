...
import {
	...
	PIECE_SELECTION_DIALOG_RESULT_TYPE,
} from './extensions/PieceSelectionDialog.js';
import {
	Chessboard,
	COLOR,
	INPUT_EVENT_TYPE,
...
class SimpleFenEditor extends Component {
	...
	async componentDidUpdate( prevProps ) {
		...
		if ( prevProps.isSelected && ! this.props.isSelected ) {
			if (
				this.chessboard &&
				this.chessboard.isPieceSelectionDialogShown()
			) {
		...
		if (
			...
			prevProps.fen !== this.props.fen &&
			this.chessboard &&
			this.state.chessboardLoaded
		) {
			try {
	...
	addSquareClickHandlers() {
		...
		const boardGroup = this.editorRef.querySelector(
			'g.board.input-enabled'
		);
		if ( ! boardGroup ) {
		...
		newBoardGroup.addEventListener( 'pointerdown', ( e ) => {
			...
			if (
				...
				if ( piece ) {
					...
					this.chessboard.showPieceSelectionDialog(
						squareName,
						( result ) => {
							if (
								result.type ===
								PIECE_SELECTION_DIALOG_RESULT_TYPE.pieceSelected
							) {
								this.chessboard.setPiece(
									result.square,
									result.piece
								);
								setTimeout( () => {
	 ...
	 * @return {string} The full, completed FEN string.
	 */
	...
	async initChessboard() {
		...
		try {
			this.chessboard = new Chessboard( this.editorRef, {
				...
				responsive: true,
				assetsUrl:
					( window.roiChessEditor?.pluginUrl || '' ) +
			...
			if ( this.editorRef ) {
				this.editorRef.innerHTML =
					'<div style="padding: 20px; text-align: center; background: #fee; border-radius: 4px; color: #c00;">⚠️ Erreur de chargement</div>';
			}
		}
	}
...
export default function Edit( props ) {
	...
	const [ fenValidation, setFenValidation ] = useState( {
		isValid: true,
		error: null,
	} );
	...
	}, [ fenValidation.isValid, isEngineEnabled, setAttributes ] );
			...
			<InspectorControls>
				<PanelBody title={ __( 'Configuration', 'roi' ) }>
					<SelectControl
						...
						options={ [
							{
								label: __( 'Sélectionner…', 'roi' ),
								value: '',
								disabled: true,
							},
							{
								label: __( 'Position initiale', 'roi' ),
								value: FEN.start,
							},
							{
								label: __( 'Échiquier vide', 'roi' ),
								value: FEN.empty,
							},
						] }
						onChange={ ( value ) => {
							if ( value ) {
								setAttributes( { fen: value } );
							}
						} }
					...
					<div className="castling-controls">
						<span className="castling-controls__label">
							{ __( 'Roques', 'roi' ) }
						</span>
							...
							<tbody>
								<tr>
									...
									<td>
										<CheckboxControl
											label="O-O"
											checked={ castlingRights.includes(
												'K'
											) }
											onChange={ ( isChecked ) =>
												handleCastleChange(
				...
				<PanelBody title={ __( "Style de l'échiquier", 'roi' ) }>
					...
					<ToggleControl
						...
						onChange={ ( value ) =>
							setAttributes( { showCoordinates: value } )
						}
						__nextHasNoMarginBottom={ true }
					/>
					<SelectControl
						...
						options={ [
							{
								label: __( 'Standard', 'roi' ),
								value: 'standard',
							},
							{ label: __( 'Staunty', 'roi' ), value: 'staunty' },
						] }
						onChange={ ( value ) =>
							setAttributes( { pieces: value } )
						}
						__next40pxDefaultSize={ true }
						__nextHasNoMarginBottom={ true }
					/>
				...
				<PanelBody title={ __( 'Mode de jeu', 'roi' ) }>
					...
					<ToggleControl
						...
						help={
							! fenValidation.isValid
								? __(
										'Le FEN doit être valide pour activer le moteur.',
										'roi'
								  )
								: ''
						}
					...
					{ isEngineEnabled && (
						<RangeControl
							...
							label={ `${ __( 'Niveau', 'roi' ) } (ELO ${
								attributes.engineElo
							})` }
							value={ attributes.engineElo }
							onChange={ ( value ) =>
								setAttributes( { engineElo: value } )
							}
							min={ 1200 }
							max={ 2800 }
							step={ 100 }
