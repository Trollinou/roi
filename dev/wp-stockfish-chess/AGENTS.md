# AGENTS — Plugin wp-stockfish (Plugin permettant de jouer contre Stockfish)

## Objectif

Ce document décrit le ou les **agents** (IA / assistants) destinés à assister le développement, la revue et la maintenance du plugin WordPress **wp-stockfish** .
...
1. **Conseiller en architecture** — proposer une organisation de fichiers et modules (classes, namespaces, prefix) adaptée à wp-stockfish.
...
- Préfixer toutes les fonctions, classes, hooks, options et meta keys par `wpsf_` ou `WPSF\` pour les namespaces PHP.
- Exemple de classe : `WPSF\Core\Member_Manager`.

### Arborescence recommandée

```
wp-content/plugins/wp-stockfish/
├─ assets/
│  ├─ css/
│  ├─ js/
│  └─ img/
...
- Prefixer les fonctions globales : `wpsf_get_member()`.
...
- Charger le textdomain `wpsf` dans l'initialisation du plugin : `load_plugin_textdomain( 'wpsf', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );`.
- Toutes les chaînes PHP doivent utiliser `__()`, `_e()`, `esc_html__()`, `esc_attr__()` etc. Exemple :

```php
// Exemple conforme
_e( "Appliquer les modifications", 'wpsf' );
```
...
if ( ! isset( $_POST['wpsf_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['wpsf_nonce'] ), 'wpsf_action' ) ) {
    wp_die( -1 );
}
```

- **Capabilities** : vérifier les capacités avant toute modification (`current_user_can( 'manage_options' )` ou une capability spécifique `wpsf_manage_members`).
...
- Déclarer des hooks publics (actions et filtres) documentés, par ex. `do_action( 'wpsf_after_member_save', $member_id );`.
- Prévoir des filtres pour personnaliser les comportements : `apply_filters( 'wpsf_member_meta', $meta );`.

---

## REST API

- Préfixer les routes : `wp-json/wpsf/v1/members`.
...
*Document version : 1.0 — Généré pour le plugin wp-stockfish.*
