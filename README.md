...
*   **Sauvegarde et Restauration du Contenu :** Une page d'administration dédiée pour exporter tout le contenu pédagogique (leçons, exercices, cours, catégories) dans un fichier `.json.gz` et le restaurer, prévenant ainsi la perte de données.
    ...
    *   **Type 14 (Cap ou pas Cap ?) :** Configuration de séries de 5 Mini-PGN avec sélection de variante (`qcm_multiple`, `qcm_oui_non`, `move`, `notation`), gestionnaire de propositions globales ou question commune, déduction automatique des coordonnées cibles pour la notation française, et prévisualisation PGN interactive.
