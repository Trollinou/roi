...
*   **Sauvegarde et Restauration ISO Complète (`Backup.php`, `CHANGELOG.md`) :**
    ...
    *   **Préservation Intégrale des Métadonnées JSON & Restauration Fiable :**
        ...
        *   Normalisation systématique des configurations d'exercices (`_roi_exercice_config`) et playlists (`_roi_cours_playlist`) en chaînes JSON valides, avec décodage multi-niveaux récursif (empêchant toute corruption issue d'un double échappement ou d'une sérialisation PHP).
        *   Désérialisation et hydratation robuste dans la metabox d'exercice (`Manager.php`), le DTO (`Exercice_Config_DTO.php`) et l'API REST (`Contenu_Controller.php`) prenant en compte de manière transparente les tableaux PHP, chaînes sérialisées ou JSON multi-échappés, garantissant le peuplement immédiat de l'ensemble des champs `FenInput` et `PgnInput`.
        *   Purge explicite des caches WordPress (`wp_cache_delete` sur `posts` et `post_meta`, `clean_post_cache`, `wp_cache_flush`) lors de la restauration pour garantir que les données rafraîchies soient immédiatement servies.
    ...
    *   **Rétrocompatibilité :** Prise en charge transparente des sauvegardes historiques au format JSON standard et décompression universelle (`gzcompress`/`gzuncompress`, `gzencode`/`gzdecode`, JSON brut).
...
*   **Refactoring & Modularisation PSR-4 (`Progression_Controller.php`, `Builder.php`) :**
    ...
    *   **Correction Warning PHP `template.php:1327` :** Sécurisation du réordonnancement des métaboxes latérales (`ordonner_metaboxes_side`) et ajout d'un filtre d'assainissement automatique sur `get_user_option("meta-box-order_{$screen->id}")` et `get_user_metadata` dans `Assets.php` pour immuniser l'administration WordPress contre les usermeta corrompus ou au format chaîne de caractères.
...
*   **Notation Française dans les Prévisualisations PGN et l'Éditeur (`chessUtils.js`, `pgn-viewer.js`, `PgnEditor.jsx`) :**
    *   **Traduction des Coups et Variantes Alternatives :** Conversion automatique en notation française (R, D, T, F, C au lieu de K, Q, R, B, N) des coups affichés dans la liste de la branche active, des boutons de sélection de variantes dans le composant de prévisualisation `PgnPreviewViewer`, et dans la modale d'édition interactive `PgnEditor`.
...
*   **Assignation de Cours Ciblés & Gestion des Groupes (`Audience.php`, `Parcours_Controller.php`, `Progression_Controller.php`, `SuiviApp.jsx`, `StudentDetailModal.jsx`) :**
    ...
    *   **Tableau des Cours & Onglets d'Audience (`Columns.php`) :** Ajout des onglets de vues au sommet de la table d'administration des cours (`views_edit-roi_cours`) : « Tous les cours », « 📚 EEF » (Méthode École d'Échecs à la Française) et « 📌 Cours assignés » avec décomptes dynamiques. Filtrage natif `pre_get_posts`, sélecteur déroulant dans `restrict_manage_posts`, et nouvelle colonne « Audience » affichant le badge `📚 EEF` pour les cours classiques ou le badge `📌 Assigné` avec les badges des groupes et la liste des élèves ciblés.
    ...
    *   **Correction de l'Erreur React #310 (« Rendered more hooks than during previous render ») (`SuiviApp.jsx`) :** Repositionnement du hook `useMemo` de calcul de `availableGroups` au sommet du composant principal au même titre que `groupStats` (avant les conditions d'affichage précoce `if (loading)` et `if (error)`), éliminant l'écart de nombre de hooks lors du passage de l'état de chargement à l'affichage des données.
