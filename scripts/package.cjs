  ...
  const findSources = (dir) => {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        if (findSources(fullPath)) return true;
...
if (hasSources) {
  ...
  console.log("ℹ️  No source files to compile in src/, skipping build step.");
}

// Load .distignore rules
const distIgnoreContent = fs.existsSync('.distignore') ? fs.readFileSync('.distignore', 'utf8') : '';
...
function shouldIgnore(relativePath) {
  ...
  for (const pattern of ignorePatterns) {
    ...
    if (normalizedPath === cleanPattern || normalizedPath.startsWith(cleanPattern + '/')) {
      return true;
    }
  }
  // Also ignore temp directories and zips
  if (normalizedPath.startsWith('dist-temp') || normalizedPath.endsWith('.zip') || normalizedPath.endsWith('.sh')) {
    return true;
  }
  return false;
}
...
try {
  ...
  console.warn("adm-zip not available directly, attempting fallback command...");
}

if (!zipCreated) {
  if (process.platform === 'win32') {
    try {
      execSync(`powershell -Command "Compress-Archive -Path '${targetDir}' -DestinationPath '${zipName}' -Force"`, { stdio: 'inherit' });
      console.log(`✓ Zip created successfully via PowerShell: ${zipName}`);
      zipCreated = true;
    } catch (pwshErr) {
      try {
        execSync(`tar -a -c -f "${zipName}" -C "${tempDir}" "${pluginSlug}"`, { stdio: 'inherit' });
        console.log(`✓ Zip created successfully via tar: ${zipName}`);
        zipCreated = true;
      } catch (tarErr) {
    ...
    try {
      execSync(`cd "${tempDir}" && zip -r "../${zipName}" "${pluginSlug}" > /dev/null`, { stdio: 'inherit' });
      console.log(`✓ Zip created successfully via zip: ${zipName}`);
      zipCreated = true;
    } catch (zipErr) {
      console.error("❌ Error: Failed to create zip via shell command.");
      process.exit(1);
    }
  }
}
